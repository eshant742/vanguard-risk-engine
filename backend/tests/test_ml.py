...
class TestMLEngine:
    ...
    def test_model_caching_returns_same_metrics(self):
        """Second call to initialize_model should use cache and return identical metrics."""
        m1 = initialize_model()
        m2 = initialize_model()
    ...
    def test_shap_explanation_values_are_floats(self):
        """SHAP contribution values in the prediction must be valid floats."""
        resp = client.post("/api/fraud/predict", json={
            "amount": 5000,
            "device_velocity": 6,
            "ip_country_match": 0,
            "time_since_last_txn": 3.5
        })
        data = resp.json()
        assert "xai_flags" in data
        assert isinstance(data["xai_flags"], list)

    def test_xai_reason_string_format(self):
        """XAI reason string should be a comma-separated list of flags."""
        resp = client.post("/api/fraud/predict", json={
            "amount": 8000,
            "device_velocity": 8,
            "ip_country_match": 0,
        ...
        if reason != "Safe" and "Allowed" not in reason:
            # If blocked, reason should list drivers or XAI flags
            assert "Amount" in reason or "Velocity" in reason or "Ip Country" in reason or "Time" in reason
