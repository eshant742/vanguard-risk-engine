    ...
    <div className="dashboard-grid">
      <div className="card" style={{ height: 'fit-content' }}>
        ...
        <form onSubmit={handleScore} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <label className="input-label">Items Kept (Last 12 Months)</label>
            <input 
              type="number" 
              className="input-box" 
              value={itemsKept}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <label className="input-label">Items Returned (Last 12 Months)</label>
            <input 
              type="number" 
              className="input-box" 
              value={itemsReturned}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <label className="input-label">Current Cart Value (INR)</label>
            <input 
              type="number" 
              className="input-box" 
              value={cartValue}
      ...
      <div className="card">
        ...
        {!loading && !result && !error && (
          <div className="empty-state">
            ...
            <div className="empty-state-title">Awaiting Data</div>
            <div className="empty-state-desc">Submit a customer profile to generate a dynamic return policy.</div>
          </div>
        )}
            ...
            {result.breakdown && (
              <div style={{ backgroundColor: 'var(--surface-input)', border: '1px solid var(--border-light)', borderRadius: '8px', padding: '1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                  <div style={{ fontSize: '0.85rem', color: 'var(--primary)', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 'bold' }}>Risk Breakdown</div>
